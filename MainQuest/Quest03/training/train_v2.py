"""추가 실험: 변환만 학습한 D와 보조 과제를 학습한 B/C를 비교한다.
...
def main():
    ...
    p.add_argument('--condition', choices=['B','C','D'], required=True)
    p.add_argument('--data', default='data/train_reviewed.jsonl')
    p.add_argument('--model', default='Qwen/Qwen2.5-0.5B-Instruct')
    p.add_argument('--revision', required=True, help='파일럿에서 고정한 기반 모델 버전')
    p.add_argument('--epochs', type=int, default=10)
    p.add_argument('--seed', type=int, default=20260923)
    p.add_argument('--accumulation', type=int, default=8)
    p.add_argument('--lr', type=float, default=2e-4)
    p.add_argument('--max-length', type=int, default=1536)
    p.add_argument('--annotation-mode', choices=['reviewed_ocr','synthetic_coordinates'], default='synthetic_coordinates')
    args = p.parse_args()
    ...
    for row in rows:
        ...
        tasks=[]
        task_targets=[('CONVERT',row['target'])]
        if args.condition != 'D':
            task_targets.append(('ANNOTATE',auxiliary))
        for task,target in task_targets:
    ...
    metadata = vars(args) | {'data_sha256':digest(args.data),'gpu':torch.cuda.get_device_name(0),
        ...
        'loss_weights':{'CONVERT':1.0 if args.condition=='D' else 0.5,'ANNOTATE':0.0 if args.condition=='D' else 0.5}, 'examples':len(rows),
        'target_tokens_per_epoch':{'main':sum(e[0][2] for e in encoded),'auxiliary':sum(e[1][2] for e in encoded if len(e)>1)}}
