...
def main():
    ...
    p.add_argument('--split',choices=['validation','test','train_probe'],required=True)
    p.add_argument('--condition',choices=['A','B','C','D'],required=True)
    ...
    # 추가 실험 계획을 먼저 고정한다. 기존 테스트는 재사용한 탐색 평가로 표기한다.
    choice=json.loads(Path(a.selection).read_text())
    if choice['revision']!=a.revision or choice['data_sha256'][a.split]!=digest(a.data):
        raise ValueError('사전 계획의 모델·데이터 지문 불일치')
    if a.condition!='A' and a.adapter not in choice['adapters'][a.condition]:
        raise ValueError('사전에 지정한 5·10회차가 아닌 모델')
    rows=read_jsonl(a.data)
    expected_split='train' if a.split=='train_probe' else a.split
